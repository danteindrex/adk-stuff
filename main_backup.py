...
async def whatsapp_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if "Body" not in body or "From" not in body:
            await monitoring_service.log_error_event("webhook_validation", "Missing required Twilio fields", {"body_keys": list(body.keys())})
            return {"status": "invalid_webhook"}
        
        # Process Twilio WhatsApp message directly
        message = {
            "text": {
                "body": body["Body"]
            },
